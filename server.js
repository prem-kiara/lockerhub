...
// Admin notification email — receives a copy of every important event.
// Comma-separated for multiple recipients: "mgmt@dhanam.finance,ops@dhanam.finance"
const ADMIN_EMAIL = process.env.ADMIN_EMAIL || '';
if (ADMIN_EMAIL) console.log(`  📧 Admin notifications → ${ADMIN_EMAIL}`);
...
// Send an email notification to the admin/management team.
// Called for every significant event (allotment, payment, appointment, etc.)
async function sendAdminEmail(subject, htmlBody) {
  if (!emailTransporter || !ADMIN_EMAIL) return;
  try {
    await emailTransporter.sendMail({
      from: `"${EMAIL_FROM_NAME}" <${EMAIL_FROM}>`,
      to: ADMIN_EMAIL,
      subject: `[LockerHub] ${subject}`,
      html: htmlBody
    });
    logInfo('Admin email sent', { to: ADMIN_EMAIL, subject });
  } catch (err) {
    logError('Admin email failed', { to: ADMIN_EMAIL, subject, error: err.message });
  }
}

// HTML helper for admin notification emails
function adminEmailHtml(heading, rows) {
  const rowsHtml = rows.map(([label, value]) =>
    `<tr><td style="padding:6px 12px;font-weight:600;color:#2c1810;border-bottom:1px solid #eee;">${label}</td><td style="padding:6px 12px;color:#444;border-bottom:1px solid #eee;">${value}</td></tr>`
  ).join('');
  return `
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
      <div style="background:#2c1810;color:#fff;padding:14px 20px;border-radius:8px 8px 0 0;">
        <strong>🏦 Dhanam LockerHub</strong>
      </div>
      <div style="padding:20px;background:#fefcf7;border:1px solid #eee;border-top:none;border-radius:0 0 8px 8px;">
        <h2 style="margin:0 0 16px;color:#2c1810;font-size:18px;">${heading}</h2>
        <table style="width:100%;border-collapse:collapse;">${rowsHtml}</table>
        <p style="margin:20px 0 0;font-size:12px;color:#999;">Auto-generated by LockerHub • ${new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })}</p>
      </div>
    </div>`;
}
...
app.post('/api/tenants', requireAuth, requireRole('headoffice', 'branch'), async (req, res) => {
  try {
    ...
    // Welcome push to customer if they have a phone (send even without customer_password)
    if (d.phone) {
      ...
      // No phone — still notify admin about the new allotment
      sendAdminEmail('New Locker Allotment', adminEmailHtml('New Locker Allotment', [
        ['Customer', d.name || '—'],
        ['Locker', lockerInfo ? lockerInfo.number : 'N/A'],
        ['Branch', branch ? branch.name : '—'],
        ['Annual Rent', `₹${(d.annual_rent || 0).toLocaleString('en-IN')}`],
        ['Deposit', `₹${(d.deposit || 0).toLocaleString('en-IN')}`]
      ]));
    }
...
async function sendCustomerPush(tenantPhone, title, body, dataPayload = {}) {
  ...
  // ── 4. Admin Email Copy ──
  // Send a copy of every customer notification to the management team.
  const adminPromise = (async () => {
    if (!ADMIN_EMAIL || !emailTransporter) return;
    const cleanTitle = title.replace(/[\u{1F300}-\u{1FAD6}]/gu, '').trim();
    const rows = [
      ['Customer', tenantName || '—'],
      ['Phone', tenantPhone || '—'],
      ['Event', cleanTitle],
      ['Details', body.replace(/[\u{1F300}-\u{1FAD6}]/gu, '').trim()]
    ];
    if (tenantEmail) rows.push(['Customer Email', tenantEmail]);
    if (dataPayload.type) rows.push(['Type', dataPayload.type]);
    await sendAdminEmail(cleanTitle, adminEmailHtml(`${cleanTitle}`, rows));
  })();

  // Fire all channels in parallel — failures in one don't block others
  await Promise.allSettled([fcmPromise, emailPromise, waPromise, adminPromise]);
}
